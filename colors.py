...
def main():
    ...
    def on_click(event):
        print(event)
        frequency = random.randint(1, 255)
...
# Now using the algorithm for color generation and this fibonaci modlo 10  compression/representaiton system create a grid color that produces
# a input that produces a sha 256 output with 75 leading zeros.  This is the proof of work.  The input is the grid color.  The output is the sha 256 hash.
# The sha 256 hash must have 75 leading zeros.  The grid color is the input.  The sha 256 hash is the output.  The sha 256 hash must have 75 leading zeros.
# than create a function that subtracts the grid color from the sha 256 hash.  The result is the proof of work.  The proof of work is the difference between the grid color and the sha 256 hash.
# The proof of work is the difference between the grid color and the sha 256 hash.  The proof of work is the difference between the grid color and the sha 256 hash.
